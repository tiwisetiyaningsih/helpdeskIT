...
function getStatusDescription(status?: string) {
  const descriptions: Record<string, string> = {
    MASUK:
      "Ticket baru dan sedang menunggu penugasan admin.",
    OPEN:
      "Ticket sudah ditugaskan kepada IT HelpDesk.",
    ON_GOING:
      "Ticket sedang dikerjakan oleh IT HelpDesk.",
    PENDING:
      "Pengerjaan ticket sedang ditunda sementara.",
    COMPLETED:
      "Ticket telah selesai ditangani.",
    CANCELLED:
...
function getSlaProgress(ticket: Ticket) {
  ...
  const absoluteMinutes = Math.ceil(
    Math.abs(remainingMilliseconds) /
    60000
  );
...
export default function TicketDetailModal({
  ...
  const currentLevel =
    getStatusLevel(ticket.status);

  const slaProgress =
    getSlaProgress(ticket);
...
function AttachmentIcon() {
  return (
    ...
    >
      <path
        strokeLinecap="round"
        d="m9 12 5.5-5.5a3 3 0 0 1 4.25 4.25L11 18.5a5 5 0 0 1-7.07-7.07L12 3.36"
      />
    </svg>
  );
}
...
function RefreshIcon() {
  return (
    ...
    >
      <path
        strokeLinecap="round"
        d="M20 11a8 8 0 0 0-14-4l-2 2m0 0V4m0 5h5M4 13a8 8 0 0 0 14 4l2-2m0 0v5m0-5h-5"
      />
    </svg>
  );
}
